%% Clear version of Pressure Diffusion in 2D - Florina, Greece
...
%% Load Data
Florina_Data  = readtable('Florina_catalog.xlsx');
Mcompl        = find(Florina_Data.Magnitude >= 1.3);
Florina_Data  = Florina_Data(Mcompl,:);
area          = find(Florina_Data.Longitude >= 21.27 & Florina_Data.Longitude <=21.47 & Florina_Data.Latitude >= 40.68 & Florina_Data.Latitude <= 40.86);
Florina_Data  = Florina_Data(area,:);
main_shock    = Florina_Data(103,:);
Florina_Data  = Florina_Data([185:end],:);
Lat   = Florina_Data.Latitude;
Long  = Florina_Data.Longitude;
M     = Florina_Data.Magnitude;
depth = Florina_Data.Depth;
yr = datetime(Florina_Data.Year,Florina_Data.Month,Florina_Data.Day,Florina_Data.Hour,Florina_Data.Min, Florina_Data.Sec);
...
data_subset_florina;
%% Domain parameters
L                       = 15*1000;      %   Length of domain  [m]
H                       = 12*1000;      %   Height of domain  [m]
...
num_faults              = 2;            %   Specify the number of discrete faults in the model (current maximum 5)
...
tend                    = 180;          %   Overall sumulated time in days (%tend=1000,Save_timestep=2000;)
...
%% Faults
%Fault 1
Faults(1).x0            = 8750;         %   x-coordinate of faults upper limit
Faults(1).y0            = -3690;        %   y-coordinate of faults upper limit
Faults(1).yend          = -7600;        %   y-coordinate of faults lower limit (x-coordinate is calculated)
Faults(1).thickness     = 0.1  *1000;    %   Thickness of the fault [m]
Faults(1).dip           = 35;           %   Dip of the fault. Measured anti-clock wise from the x-axis [deg]
Faults(1).azimuth       = 323;          %   Azimuth of the fault. Measured clock wise from the y-axis [deg]

%Fault 2
Faults(2).x0            = 8450;        %   x-coordinate of faults upper limit
Faults(2).y0            = -2000;        %   y-coordinate of faults upper limit
Faults(2).yend          = -4800;       %   y-coordinate of faults lower limit (x-coordinate is calculated)
Faults(2).thickness     = 0.05*1000;     %   Thickness of the fault [m]
Faults(2).dip           = 80;          %   Dip of the fault. Measured anti-clock wise from the x-axis [deg]
Faults(2).azimuth       = 170;           %   Azimuth of the fault. Measured clock wise from the y-axis [deg]
...
Material_mat(3,:)       =  [   60    0     0.05    1e-11      35    0.1];   %fault 2

P_input = [0 2.1e-5 4.3e-5 3.7e-5 11e-5 8e-5 7e-5 18e-5]; %Calibrated Q0
...
% Assign Q0 to point where main aftershocks are observed
 for i=2:nx-1
   for j=2:ny-1  
       if ((ms_final(1,2)-cs1_final(2,2))*1000 >= (i-1)*dx && (ms_final(1,2)-cs1_final(2,2))*1000 <= (i)*dx && -main_shock.Depth*1000 >= -(ny-(j))*dy && -main_shock.Depth*1000 <= -(ny-(j+1))*dy)
           P_matrix(i,j)=2;
       end
   end
 end
 ...
 for i=2:nx-1
   for j=2:ny-1  
       if ((p1_final(201,2)-cs1_final(2,2))*1000 >= (i-1)*dx && (p1_final(201,2)-cs1_final(2,2))*1000 <= (i)*dx && -depth(201)*1000 >= -(ny-(j))*dy && -depth(201)*1000 <= -(ny-(j+1))*dy)
           P_matrix(i,j)=3;
       end
   end
  end
  ...
  for i=2:nx-1
   for j=2:ny-1  
       if ((p1_final(243,2)-cs1_final(2,2))*1000 >= (i-1)*dx && (p1_final(243,2)-cs1_final(2,2))*1000 <= (i)*dx && -depth(243)*1000 >= -(ny-(j))*dy && -depth(243)*1000 <= -(ny-(j+1))*dy)
           P_matrix(i,j)=4;
       end
   end
  end
  ...
  for i=2:nx-1
   for j=2:ny-1  
       if ((p1_final(313,2)-cs1_final(2,2))*1000 >= (i-1)*dx && (p1_final(313,2)-cs1_final(2,2))*1000 <= (i)*dx && -depth(313)*1000 >= -(ny-(j))*dy && -depth(313)*1000 <= -(ny-(j+1))*dy)
           P_matrix(i,j)=5;
       end
   end
  end
  ...
  for i=2:nx-1
   for j=2:ny-1  
       if ((p1_final(426,2)-cs1_final(2,2))*1000 >= (i-1)*dx && (p1_final(426,2)-cs1_final(2,2))*1000 <= (i)*dx && -depth(426)*1000 >= -(ny-(j))*dy && -depth(426)*1000 <= -(ny-(j+1))*dy)
           P_matrix(i,j)=6;
       end
   end
  end
  ...
  for i=2:nx-1
   for j=2:ny-1  
       if ((p1_final(498,2)-cs1_final(2,2))*1000 >= (i-1)*dx && (p1_final(498,2)-cs1_final(2,2))*1000 <= (i)*dx && (-depth(498)-0.3)*1000 >= -(ny-(j))*dy && (-depth(498)-0.3)*1000 <= -(ny-(j+1))*dy)
           P_matrix(i,j)=7;
       end
   end
  end
  ...
  for i=2:nx-1
   for j=2:ny-1  
       if ((p1_final(717,2)-cs1_final(2,2))*1000 >= (i-1)*dx && (p1_final(717,2)-cs1_final(2,2))*1000 <= (i)*dx && (-depth(717))*1000 >= -(ny-(j))*dy && (-depth(717))*1000 <= -(ny-(j+1))*dy)
           P_matrix(i,j)=8;
       end
   end
  end
...
P_source_1 = zeros(nx,ny); % Fluid source pressure field (impermeable source is handled in initial conditions)
P_source_2 = zeros(nx,ny);
P_source_3 = zeros(nx,ny);
P_source_4 = zeros(nx,ny);
P_source_5 = zeros(nx,ny); 
P_source_6 = zeros(nx,ny); 
P_source2 = zeros(nx,ny); 
P_source3 = zeros(nx,ny);
P_source4 = zeros(nx,ny); 
P_source5 = zeros(nx,ny); 
P_source6 = zeros(nx,ny); 
P_source7 = zeros(nx,ny); 
 
parameter_florina
P_old = zeros(nx,ny);               % Solution field
kc0 = ones(nx,ny);                  % Permeability matrix (m^2)
kc = zeros(nx,ny);                   % Permeability matrix (m^2)
...
while time<tot_time
  ...
  if time_count==int16(53/dt*Day) || time_count==int16(58/dt*Day) || time_count==int16(120/dt*Day) || time_count==int16(124/dt*Day) || time_count==int16(127/dt*Day) || time_count==int16(162/dt*Day)
    theal = 0.*theal;
    theal2 = 0.*theal2;
  end
  ...
  for i=1:nx
    for j=1:ny
        ...
        if (material(i,j)==1 || material(i,j)==2 || material(i,j)==3)
        ...
        elseif time_count>int16(52/dt*Day) && time_count<=int16(57/dt*Day)
            P_source(i,j)=P_source2(i,j).*exp(-zeta.*theal2(i,j));
            kc(i,j)=kc0(i,j)+ (kc0(i,j)*100)*exp(-alpha(i,j).*theal2(i,j));
            P_source3(i,j) = P_source(i,j)+P_source_2(i,j);

        elseif time_count>int16(57/dt*Day) && time_count<=int16(119/dt*Day)
            P_source(i,j)=P_source3(i,j).*exp(-zeta.*theal2(i,j));
            kc(i,j)=kc0(i,j)+ (kc0(i,j)*100)*exp(-alpha(i,j).*theal2(i,j));
            P_source4(i,j) = P_source(i,j)+P_source_3(i,j);

        elseif time_count>int16(119/dt*Day) && time_count<=int16(123/dt*Day)
            P_source(i,j)=P_source4(i,j).*exp(-zeta.*theal2(i,j));
            kc(i,j)=kc0(i,j)+ (kc0(i,j)*100)*exp(-alpha(i,j).*theal2(i,j));
            P_source5(i,j) = P_source(i,j)+P_source_4(i,j);

        elseif time_count>int16(123/dt*Day) && time_count<=int16(126/dt*Day)
            P_source(i,j)=P_source5(i,j).*exp(-zeta.*theal2(i,j));
            kc(i,j)=kc0(i,j)+ (kc0(i,j)*100)*exp(-alpha(i,j).*theal2(i,j));
            P_source6(i,j) = P_source(i,j)+P_source_5(i,j);
        
        elseif time_count>int16(126/dt*Day) && time_count<=int16(161/dt*Day)
            P_source(i,j)=P_source6(i,j).*exp(-zeta.*theal2(i,j));
            kc(i,j)=kc0(i,j)+ (kc0(i,j)*100)*exp(-alpha(i,j).*theal2(i,j));
            P_source7(i,j) = P_source(i,j)+P_source_6(i,j);
        
        else
            P_source(i,j)=P_source7(i,j).*exp(-zeta.*theal2(i,j));
            kc(i,j)=kc0(i,j)+ (kc0(i,j)*100)*exp(-alpha(i,j).*theal2(i,j));
        end
        end
       end
