%% Clear version of Pressure Diffusion in 2D - Thiva, Greece
...
%% Load Data
Thiva_Data=readtable('S1_thiva_relocated_catalogue.xlsx');
Mcompl = find(Thiva_Data.Magnitude >= 1.6);
Thiva_Data = Thiva_Data(Mcompl,:);
save('Thiva_Data.mat','Thiva_Data');
load('Thiva_Data.mat') 
Lat = Thiva_Data.Latitude;
Long= Thiva_Data.Longitude;
M = Thiva_Data.Magnitude;
depth = Thiva_Data.Depth;
yr = datetime(Thiva_Data.Year,Thiva_Data.Month,Thiva_Data.Day,Thiva_Data.Hour,Thiva_Data.Min, Thiva_Data.Sec);
...
tend                    = 95;          %   Overall sumulated time in days (%tend=1000,Save_timestep=2000;)
...
%% Faults
%Fault 1
Faults(1).x0            = 11050;        %   x-coordinate of faults upper limit
Faults(1).y0            = -6600;        %   y-coordinate of faults upper limit
Faults(1).yend          = -11500;       %   y-coordinate of faults lower limit (x-coordinate is calculated)
Faults(1).thickness     = 0.05*1000;    %   Thickness of the fault [m]
Faults(1).dip           = 60;           %   Dip of the fault. Measured anti-clock wise from the x-axis [deg]
Faults(1).azimuth       = 210;          %   Azimuth of the fault. Measured clock wise from the y-axis [deg]

%Yliki Fault
Faults(2).x0            = 12100;        %   x-coordinate of faults upper limit
Faults(2).y0            = -8000;        %   y-coordinate of faults upper limit
Faults(2).yend          = -12500;       %   y-coordinate of faults lower limit (x-coordinate is calculated)
Faults(2).thickness     = 0.1*1000;     %   Thickness of the fault [m]
Faults(2).dip           = 65;           %   Dip of the fault. Measured anti-clock wise from the x-axis [deg]
Faults(2).azimuth       = 210;          %   Azimuth of the fault. Measured clock wise from the y-axis [deg]

%Fault 2
Faults(3).x0            = 9450;         %   x-coordinate of faults upper limit
Faults(3).y0            = -7500;        %   y-coordinate of faults upper limit
Faults(3).yend          = -10500;       %   y-coordinate of faults lower limit (x-coordinate is calculated)
Faults(3).thickness     = 0.1*1000;     %   Thickness of the fault [m]
Faults(3).dip           = 65;           %   Dip of the fault. Measured anti-clock wise from the x-axis [deg]
Faults(3).azimuth       = 30;           %   Azimuth of the fault. Measured clock wise from the y-axis [deg]
...
P_input = [0 2e-5 15e-5 3e-5];     %Calibrated Q0 
...
for k=1:num_faults
  ...
  for i=1:nx
   for j=1:ny  
       xpos = i*dx;
       ypos = -(ny-j)*dy;
       dist_to_fault(i,j) = abs((yend-y0)*xpos-(xend-x0)*ypos+xend*y0-yend*x0)/sqrt((yend-y0)^2+(xend-x0)^2);
       if (dist_to_fault(i,j) <= Faults(k).thickness && ypos >= Faults(k).yend && ypos <= Faults(k).y0)
            material(i,j)=1+k;
       end
   end
  end
...
% Assign Q0 to point where main aftershocks are observed
 for i=2:nx-1
   for j=2:ny-1  
       if ((p1_final(1,2))*1000 >= (i-1)*dx && (p1_final(1,2))*1000 <= (i)*dx && -depth(af1(1))*1000 >= -(ny-(j))*dy && -depth(af1(1))*1000 <= -(ny-(j+1))*dy)
           P_matrix(i,j)=2;
       end
   end
 end
  ...
  for i=2:nx-1
   for j=2:ny-1  
       if ((p1_final(114,2)*1000 >= (i-1)*dx && (p1_final(114,2))*1000 <= (i)*dx && -depth(af1(114))*1000 >= -(ny-(j))*dy && -depth(af1(114))*1000 <= -(ny-(j+1))*dy)
           P_matrix(i,j)=3;
       end
   end
  end
  ...
  for i=2:nx-1
   for j=2:ny-1  
       if ((p1_final(203,2))*1000 >= (i-1)*dx && (p1_final(203,2))*1000 <= (i)*dx && -depth(af1(203))*1000 >= -(ny-(j))*dy && -depth(af1(203))*1000 <= -(ny-(j+1))*dy)
           P_matrix(i,j)=4;
       end
   end
  end
...
while time<tot_time
  ...
  time =time+dt;
  n_iter= 0;
  if time_count==int16(10/dt*Day) || time_count==int16(15/dt*Day)
    theal = 0.*theal;
    theal2 = 0.*theal2;
  end
  ...
  for i=1:nx
    for j=1:ny
        ...
        elseif time_count>int16(9/dt*Day) && time_count<=int16(14/dt*Day)
